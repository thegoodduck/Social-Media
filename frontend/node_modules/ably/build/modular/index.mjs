...
async function encode(msg, options) {
  ...
  const {
    data,
    encoding
  } = encodeData(msg.data, msg.encoding, isNativeDataType);
  msg.data = data;
  msg.encoding = encoding;
  ...
  async processMessage(message) {
    ...
    switch (message.action) {
      ...
      case actions.OBJECT_SYNC: {
        ...
        await Promise.all(objectMessages.map((om) => this.client._objectsPlugin.ObjectMessage.decode(om, this.client, this.logger, logger_default, utils_exports, format)));
...
var RestAnnotations = class {
  ...
  async publish(msgOrSerial, annotationValues) {
    ...
    const client = this.channel.client, options = client.options, format = options.useBinaryProtocol ? "msgpack" /* msgpack */ : "json" /* json */, headers = defaults_default.defaultPostHeaders(client.options, {
      format
    }), params = {};
    mixin(headers, client.options.headers);
...
function fromDeserialized(deserialized, presenceMessagePlugin, annotationsPlugin, objectsPlugin) {
  ...
  if (objectsPlugin && deserialized.state) {
    state = objectsPlugin.ObjectMessage.fromValuesArray(deserialized.state, utils_exports, MessageEncoding);
  }
...
var Protocol = class extends eventemitter_default {
  ...
  send(pendingMessage) {
    ...
    if (this.logger.shouldLog(logger_default.LOG_MICRO)) {
      logger_default.logActionNoStrip(this.logger, logger_default.LOG_MICRO, "Protocol.send()", "sending msg; " + stringify(pendingMessage.message, this.transport.connectionManager.realtime._RealtimePresence, this.transport.connectionManager.realtime._Annotations, this.transport.connectionManager.realtime._objectsPlugin));
    }
    ...
    try {
      ...
      if (items && items.length)
        for (let i = 0; i < items.length; i++)
          this.onProtocolMessage(fromDeserialized(items[i], this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations, this.connectionManager.realtime._objectsPlugin));
    } catch (e) {
...
var WebSocketTransport = class extends transport_default {
  ...
  onWsData(data) {
    try {
      this.onProtocolMessage(deserialize(data, this.connectionManager.realtime._MsgPack, this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations, this.connectionManager.realtime._objectsPlugin, this.format));
    } catch (e) {
